...
namespace App\Modules\Drive\Persistence\Dao\Vehicle;
                           ...
                           $status = null, $mountStatus = null)
    {
        $detail_id =
            \DB::table($this->table)->insertGetId([
                ...
                'status' => Detail::DETAIL_NORMAL_STATUS,
                'state_percent' => 100,
                'mount_status' => Detail::UNMOUNTED_STATUS,
    ...
    public function getMountedByVehicle($vehicle_id)
    {
        $details =
            \DB::table($this->table . ' AS d')
                ...
                ->where('d.mount_status', Detail::MOUNTED_STATUS)
                ->get();
    ...
    public function getUnmountedDetails($driver_id)
    {
        $details =
            \DB::table($this->table . ' AS d')
                ...
                ->where('d.mount_status', Detail::UNMOUNTED_STATUS)
                ->get();
    ...
    public function findDetailForUnmount($detail_id)
    {
        $detail =
            \DB::table($this->table)
                ...
                ->where('mount_status', Detail::MOUNTED_STATUS)
                ->where('vehicle_id', null)
                ->find($detail_id);
