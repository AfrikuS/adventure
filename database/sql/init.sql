...
CREATE TABLE IF NOT EXISTS `railway_station_licenses` (
    `id` INT UNSIGNED NOT NULL,
    `level` INT UNSIGNED NOT NULL,
    `status` VARCHAR(255) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `railway_transit_trains` (
    `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `trade_lazy_trades` (
    `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
    `hero_id` INT UNSIGNED NOT NULL,
    `resource_code` VARCHAR(255) NOT NULL,
    `resource_amount` INT UNSIGNED NOT NULL,
    `resource_price` INT UNSIGNED NOT NULL,
    PRIMARY KEY (id),
    FOREIGN KEY (hero_id) REFERENCES hero_resources(id)
);

CREATE TABLE IF NOT EXISTS `trade_resources_prices` (
    `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
    `resource_code` VARCHAR(255) NOT NULL,
    `railway_price` INT UNSIGNED NOT NULL,
    `sea_price` INT UNSIGNED NOT NULL,
    PRIMARY KEY (id)
);
...
---                              NPC
-- one old npc-table in migrations
...
CREATE TABLE IF NOT EXISTS `npc_conductors_heroes_relations` (
    `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
    `hero_id` INT UNSIGNED NOT NULL,
    `conductor_npc_id` INT UNSIGNED NOT NULL,
    `respect_level` INT UNSIGNED NOT NULL,

    `drain_oil_price` INT UNSIGNED NOT NULL,
    `drain_petrol_price` INT UNSIGNED NOT NULL,

    PRIMARY KEY (id),
    FOREIGN KEY (hero_id) REFERENCES hero_resources(id),
    FOREIGN KEY (conductor_npc_id) REFERENCES npc_characters(id),
    UNIQUE KEY `unique_conductor_hero_relation` (`hero_id`,`conductor_npc_id`)
);
ALTER TABLE `npc_conductors_heroes_relations` ADD UNIQUE (`hero_id`,`conductor_npc_id`);


CREATE TABLE IF NOT EXISTS `npc_conductor_sessions` (
    `hero_id` INT UNSIGNED NOT NULL,
    `conductor_id` INT UNSIGNED NOT NULL,
    ...
    PRIMARY KEY (hero_id),
    FOREIGN KEY (hero_id) REFERENCES hero_resources(id),
    FOREIGN KEY (conductor_id) REFERENCES npc_characters(id),
    FOREIGN KEY (train_id) REFERENCES railway_transit_trains(id)
);
...
CREATE TABLE IF NOT EXISTS `hero_equipment` (
    ...
    `oil_distillator_level` INT UNSIGNED NOT NULL,
...
--    `oil_amount` INT UNSIGNED NOT NULL,

CREATE TABLE IF NOT EXISTS `hero_resource_store` (
    `hero_id` INT UNSIGNED NOT NULL,

    `oil_capacity_level` INT UNSIGNED NOT NULL,
    `oil_capacity_amount` INT UNSIGNED NOT NULL,
    `oil_amount` INT UNSIGNED NOT NULL,

    `petrol_capacity_level` INT UNSIGNED NOT NULL,
    `petrol_capacity_amount` INT UNSIGNED NOT NULL,
    `petrol_amount` INT UNSIGNED NOT NULL,

    `water_capacity_level` INT UNSIGNED NOT NULL,
    `water_capacity_amount` INT UNSIGNED NOT NULL,
    `water_amount` INT UNSIGNED NOT NULL,
